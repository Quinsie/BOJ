...
vector <char> v;
void change(char map[][3], char dir) {
	char tmp[3][3];
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			tmp[i][j] = map[i][j];
		...
		for (int i = 0; i < m; i++) {
			...
			case 'R':
				...
				if (dir == '+') {
					...
					swap(back[0][0], down[0][2]);
					swap(back[1][0], down[1][2]);
					swap(back[2][0], down[2][2]); // down 场
					swap(back[0][0], back[2][0]);
					swap(down[0][2], front[0][2]);
					swap(down[1][2], front[1][2]);
					swap(down[2][2], front[2][2]); // front 场
				}break;
			...
			case 'L':
				...
				if (dir == '+') {
					...
					swap(back[0][2], back[2][2]); // back 场
					swap(down[0][0], front[0][0]);
					swap(down[1][0], front[1][0]);
					swap(down[2][0], front[2][0]); // down 场
					swap(front[0][0], up[0][0]);
					swap(front[1][0], up[1][0]);
					swap(front[2][0], up[2][0]); // front 场
					swap(up[0][0], up[2][0]); // up 场
				}
				else {
